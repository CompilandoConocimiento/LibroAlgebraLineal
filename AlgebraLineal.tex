    ...
    \chapter{Álgebra Matricial}
        ...
        \section{Producto de Matrices}
            ...
            \subsection{Propiedades}
                Sea $A \in M_{m \times n}(\mathbb{F})$ y $B \in M_{n \times p}(\mathbb{F})$
                y $\alpha, \beta \in \mathbb{F}$ entonces tenemos que:
        ...
        \section{Transpuesta de una Matriz}
            ...
            \subsection{Definición}
                ...
                \begin{equation}
                    transpuesta(A) = A^T = \bVector{ A_{j, i} }_{i, j = 1}^{n, m}
                \end{equation}
            ...
            \clearpage
            \subsection{Propiedades}
                Sea $A,B \in M_{m \times n}(\mathbb{F})$ y $\alpha \in \mathbb{F}$ entonces
                tenemos que:
                ...
                \begin{itemize}
                    ...
                    \item Sea $A \in M_{m \times n}(\mathbb{F})$ y $B \in M_{n \times p}(\mathbb{F})$
                        ...
                        \begin{SmallIndentation}[1em]
                            ...
                            Esta propiedad es muy interesante, primero veamos que ambas
                            matrices que queremos que sean iguales tienen las mismas dimensiones:
                            ...
                            Veamos que $[(AB)^T]_{i, j} = [(AB)]_{j, i} = \sum_{k=1}^{n} A_{j, k}B_{k, i}$
                            Y veamos también que $[B^T A^T]_{i, j} = \sum_{k=1}^{n} B^T_{i, k} A^T_{k, j}$
                            Exploremos más la última expresión y es que si te das cuenta
                            $\sum_{k=1}^{n} B^T_{i, k} A^T_{k, j} = \sum_{k=1}^{n} B_{k, i} A_{j, k}$
                            Y finalmente ya que $ab=ba$ tenemos que $\sum_{k=1}^{n} A_{j, k}B_{k, i}$
                            Así que hemos demostrado que:
                            $[(AB)^T]_{i, j} = \sum_{k=1}^{n} A_{j, k}B_{k, i} = [B^T A^T]_{i, j}$
